-- Apparently, require() doesn't work for plugins?????
-- Below is a copy-paste of all of selene and selene.parser.
-- split() was changed to not trim away all whitespace, more info in a comment.
...
do
    local function require_selene_parser()
        local selenep = {}
...
local function split(self, sep)
  ...
  if self:len() > 0 then
    ...
    while start do
      -- DIFFERENT FROM SELENE: This was trim(self:sub(searchIndex, start - 1)) here previously, but it caused whitespace to not be preserved, destroying coloring for non-selene lines.
      t[tblIndex] = self:sub(searchIndex, start - 1)
      tblIndex = tblIndex + 1
      searchIndex = stop + 1
...
return selenep
    ...
    selene.parser = require_selene_parser()
end
...
function OnSetText(uri, text)
    local selened = selene.parse(text, false)
    if selened == text then
        return nil
    end
